...
import {  visiteStatusTypes } from '../data/data';
...
export function DataTableToolbar<TData>({ table }: DataTableToolbarProps<TData>) {
        ...
        const userOptions = useMemo(
            () =>
              (users ?? []).map((u: any) => ({
                ...
                value: u.id, // IMPORTANT : doit matcher la valeur stockée dans la colonne acteur
              })),
            [users]
          );
        ...
        if (savedDateRange && savedDateRange !== "undefined") {
            try {
                ...
                if (stroredDateRange) {
                    ...
                    table.getColumn('dateVisite')?.setFilterValue(parsedDateRange);
                }
            } catch (error) {
    ...
    const handleDateRangeChange = (newDateRange: DateRange | undefined) => {
        if (newDateRange?.from != null && newDateRange?.to != null) {
            ...
            table.getColumn('dateVisite')?.setFilterValue(newDateRange);
    ...
    const handleResetFilters = () => {
        ...
        table.getAllColumns().forEach(column => {
            if (column.getCanFilter()) {
                localStorage.removeItem(`visite-filters-${column.id}`);
            }
        });
    };
                ...
                <div className="flex gap-x-2">
                    ...
                    {table.getColumn('status') && (
                        <DataTableFacetedFilter
                            ...
                            title="Statut"
                            options={visiteStatusTypes.map((t) => ({ ...t }))}
                        />
                    )}
                   
                    {table.getColumn('acteur') && (
                        <DataTableFacetedFilter
                            column={table.getColumn('acteur')}
                             title="Visiteur"
                              options={userOptions}
                        />
                    )}
