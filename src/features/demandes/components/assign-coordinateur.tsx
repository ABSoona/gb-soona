                ...
                >
                  {visiteursWithDistance.map((visiteur) => (
                    <Label
                      key={visiteur.id}
                      htmlFor={visiteur.id}
                      className={`w-full flex gap-3 items-start rounded-lg border p-3 text-left text-sm transition-all cursor-pointer ${selectedVisiteurId === visiteur.id ? "bg-muted" : "hover:bg-accent"
                        }`}
                    >
